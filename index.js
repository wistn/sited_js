...
// 2. SiteD插件容器/多多猫安卓版设置中有开发者模式开关，控制运行插件后在files文件夹里是否生成 sited_log.txt, sited_error.txt, sited_print.txt文件。多多猫里默认为假，本版引擎默认为假即不生成。如要开启生成，下行改为真值，作用于 lib/org_noear_siteder_dao_Setting.js;
global.isDeveloperModel = false;
...
// 4. Log模块( lib/android_util_Log.js)是本版引擎模仿安卓logcat功能转储消息日志，生成到 files/logcat_stdout.txt，显示消息日志过程会比 files文件夹里其他日志文件更加丰富。默认不生成，如要开启生成，下行改为真值，作用于 lib/android_util_Log.js。
global.logcatDump = false;
// 5. 上面1项为真（生成）的前提下，其中VERBOSE类型日志消息写入logcat_stdout文件时，如要同时console.log打印（每条消息开头部分）到运行本引擎的控制台，取消以下VERBOSE_log注释。也可以取消 files/VERBOSE_trace的注释来打印堆栈跟踪。
